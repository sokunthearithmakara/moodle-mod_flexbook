...
/**
 * Privacy API implementation for the Flexbook activity.
 *
...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $collection->add_database_table("flexbook_completion", [
            ...
            "lastviewed" => "privacy:metadata:lastviewed",
            "details" => "privacy:metadata:details",
        ], "privacy:metadata:flexbook_completion");

        $collection->add_database_table("flexbook_log", [
            "userid" => "privacy:metadata:userid",
            "timecreated" => "privacy:metadata:timecreated",
            ...
            "intg4" => "privacy:metadata:intg4",
            "intg5" => "privacy:metadata:intg5",
            "intg6" => "privacy:metadata:intg6",
            "char1" => "privacy:metadata:char1",
            "char2" => "privacy:metadata:char2",
            "char3" => "privacy:metadata:char3",
            "char4" => "privacy:metadata:char4",
            "char5" => "privacy:metadata:char5",
            "char6" => "privacy:metadata:char6",
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        $sql = "SELECT ctx.id
                ...
                JOIN {flexbook} iv ON iv.id = cm.instance
                JOIN {flexbook_completion} ivc ON ivc.cmid = cm.id
        ...
        $sql = "SELECT ctx.id
                ...
                JOIN {modules} m ON m.id = cm.module AND m.name = :modname
                JOIN {flexbook} iv ON iv.id = cm.instance
                JOIN {flexbook_log} ivc ON ivc.cmid = cm.id
                WHERE ivc.userid = :userid";
        $contextlist->add_from_sql($sql, $params);
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $sql = "SELECT d.userid
                ...
                JOIN {flexbook} iv ON iv.id = cm.instance
                JOIN {flexbook_completion} d ON d.cmid = cm.id
        ...
        $sql = "SELECT d.userid
                ...
                JOIN {modules} m ON m.id = cm.module AND m.name = :modulename
                JOIN {flexbook} iv ON iv.id = cm.instance
                JOIN {flexbook_log} d ON d.cmid = cm.id
                WHERE cm.id = :instanceid";
        $userlist->add_from_sql("userid", $sql, $params);
    }
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        $sql = "SELECT c.id AS contextid, ivl.*
                ...
                JOIN {course_modules} cm ON cm.id = c.instanceid
                JOIN {flexbook} iv ON iv.id = cm.instance
                JOIN {flexbook_log} ivl ON ivl.cmid = iv.id
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        $sql = "SELECT ivc.id
                FROM {flexbook_completion} ivc
                JOIN {course_modules} cm ON cm.id = ivc.cmid
                JOIN {context} c ON c.instanceid = cm.id
                WHERE c.id = :contextid";
        $completionids = $DB->get_fieldset_sql($sql, $params);
        if (!empty($completionids)) {
            $DB->delete_records_list('flexbook_completion', 'id', $completionids);
        }

        $sql = "SELECT ivl.id
                FROM {flexbook_log} ivl
                JOIN {course_modules} cm ON cm.id = ivl.cmid
                JOIN {context} c ON c.instanceid = cm.id
                WHERE c.id = :contextid";
        $logids = $DB->get_fieldset_sql($sql, $params);
        if (!empty($logids)) {
            $DB->delete_records_list('flexbook_log', 'id', $logids);
        }
    }
