...
$PAGE->add_body_class(($CFG->branch >= 500 ? ' bs-5' : '') . ' path-mod-interactivevideo');
$PAGE->set_url('/mod/flexbook/report.php', ['id' => $cm->id, 'embed' => $embed]);
...
$items = \mod_flexbook\util::get_items($cm->id, $context->id);
...
$items = array_filter($items, function ($item) use ($reportables) {
...
$datafortemplate = [
    ...
    "manualcompletion" => 1,
    "settingurl" => has_capability('mod/flexbook:edit', $context)
        ? new moodle_url('/course/modedit.php', ['update' => $cm->id]) : '',
    "reporturl" => '',
    "interactionsurl" => has_capability('mod/flexbook:edit', $context)
